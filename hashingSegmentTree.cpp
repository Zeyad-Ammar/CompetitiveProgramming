...
struct HashingSegmentTree{//use it to check palindrom of a string
private:
...
public:
    HashingSegmentTree(int n)//pass n => the size of the string
    {
        sz=1;
        while(sz<=n)sz*=2;
        seg=vector<pair<int,int>>(sz*2);
    }
    void update(int idx,char ch)//pass the idx of ur update char, and the new value of it.
    {
...
bool isPalindrome(HashingSegmentTree &a,HashingSegmentTree &b,int &l,int &r,int &n)//pass hashing segment tree on the string and its reverse.
//and the l,r of the string from main to check palindrom
{
